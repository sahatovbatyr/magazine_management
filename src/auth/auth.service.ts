...
export class AuthService {
  ...
  public getPayload(request: Request): JwtPayload {
    ...
    try {
      ...
      if (err instanceof Error) {
        throw new UnauthorizedException(`Unautorized user. Token verification failed: ${err.message}`);
      }
      throw new UnauthorizedException(`Unautorized user. Token verification failed`);
  ...
  private async verifyUser(userDto: UserLoginDto): Promise<User> {
    ...
    const isValid = await this.userService.isPasswordValid(userDto.password, hashed);
